...
/**
 * Open verified.db, run migrations, backfill email_hmac, and return the
 * data-access API. `hmacKey` comes from loadSecrets() — this module must
 * not read OTP_HMAC_KEY from process.env (runbook grep contract).
 *
 * @param {string} hmacKey
 */
function createBotDb(hmacKey) {
  ...
  backfillEmailHmac({ db, hmacKey, log });
  ...
  function hashEmail(email) {
    return emailHmac(hmacKey, email);
  }
